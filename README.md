
# study-resources

Initial commit
\# Study Resource Website



Welcome to the \*\*Study Resource Website\*\*, a project designed to provide summarized study materials and tips for various subjects.  

This website is perfect for students looking for a quick reference and links to useful resources for self-learning.



---



\## Subjects Covered



\### \*\*Economics\*\*

\- Key concepts: supply \& demand, market structures, fiscal \& monetary policies  

\- Suggested topics to focus on: GDP, inflation, unemployment, global trade



\*\*Useful Economics Links:\*\*

\- \[World Bank - Global Economic Data](https://www.worldbank.org/en/research)  

\- \[International Monetary Fund (IMF) Research](https://www.imf.org/en/Publications)  

\- \[Investopedia Economics Section](https://www.investopedia.com/economics-4689743)  

\- \[The Economist – Economic \& Financial News](https://www.economist.com/)  

\- \[Khan Academy – Microeconomics \& Macroeconomics](https://www.khanacademy.org/economics-finance-domain)



---



\### \*\*Data Science\*\*

\- Learn Python programming basics, data analysis with Pandas, and data visualization with Matplotlib \& Seaborn  

\- Topics include: statistics, machine learning concepts, and exploratory data analysis



\*\*Useful Data Science Links:\*\*

\- \[Kaggle Learn](https://www.kaggle.com/learn)  

\- \[DataCamp](https://www.datacamp.com/)  

\- \[Coursera – Data Science Courses](https://www.coursera.org/browse/data-science)  

\- \[edX – Data Science Programs](https://www.edx.org/learn/data-science)  

\- \[Towards Data Science (Articles \& Tutorials)](https://towardsdatascience.com/)



---



\### \*\*Mathematics\*\*

\- Important topics: algebra, calculus, probability, statistics, and formulas for quick reference  

\- Focus on practice problems and visualizing concepts



\*\*Useful Mathematics Links:\*\*

\- \[Paul’s Online Math Notes](https://tutorial.math.lamar.edu/)  

\- \[Khan Academy – Math Courses](https://www.khanacademy.org/math)  

\- \[Wolfram Alpha – Solve \& Explore Math Problems](https://www.wolframalpha.com/)  

\- \[Art of Problem Solving](https://artofproblemsolving.com/)  

\- \[Math is Fun](https://www.mathsisfun.com/)



---



\### \*\*Languages\*\*

\- Tips for learning Italian, Spanish, German, Polish, Finnish, and more  

\- Focus on vocabulary, grammar, and conversational practice



\*\*Useful Language Learning Links:\*\*

\- \[Duolingo](https://www.duolingo.com/)  

\- \[BBC Languages](https://www.bbc.co.uk/languages)  

\- \[Memrise](https://www.memrise.com/)  

\- \[italki – Find Tutors](https://www.italki.com/)  

\- \[Clozemaster – Vocabulary Practice](https://www.clozemaster.com/)



---



\## Study Tips



\- Break study sessions into 25–30 minute intervals with short breaks  

\- Focus on \*\*understanding concepts\*\*, not just memorizing  

\- Create summary notes and highlight key points  

\- Practice problems regularly for subjects like Math \& Data Science  

\- Review topics multiple times for long-term retention  

\- Use online resources to supplement textbooks and lectures  



---



\## Further Reading \& References



\*\*Economics \& Global Studies\*\*

\- \[OECD – Data \& Reports](https://www.oecd.org/)  

\- \[Federal Reserve Education](https://www.federalreserveeducation.org/)  

\- \[Economic Times – Global Economy](https://economictimes.indiatimes.com/)



\*\*Data Science \& Technology\*\*

\- \[Python Official Documentation](https://docs.python.org/3/)  

\- \[Stack Overflow – Q\&A](https://stackoverflow.com/)  

\- \[GitHub – Open Source Data Science Projects](https://github.com/topics/data-science)



\*\*Mathematics \& Statistics\*\*

\- \[Khan Academy Statistics \& Probability](https://www.khanacademy.org/math/statistics-probability)  

\- \[StatTrek – Online Statistics Resources](https://stattrek.com/)  

\- \[MathWorld – Wolfram](https://mathworld.wolfram.com/)



\*\*Language Learning \& Linguistics\*\*

\- \[Omniglot – Languages \& Writing Systems](https://www.omniglot.com/)  

\- \[LingQ – Language Learning Platform](https://www.lingq.com/)  

\- \[Language Transfer – Free Audio Lessons](https://www.languagetransfer.org/)



---



\## How to View



You can view the live website here:  

\[Live Website](https://xthaaman0-beep.github.io/study-rsources/)



---



\## Technologies Used



\- \*\*HTML\*\* – Structure of the website  

\- \*\*CSS\*\* – Styling and layout  

\- Optional: Add images, links, and future interactive features  



---



Feel free to explore the code, use the resources, and contribute ideas or improvements.  

This project is meant to grow over time with more content and resources.

---



\## Quick Study Cheat Sheet



\### \*\*Economics\*\*

\- \*\*Supply \& Demand\*\*

&nbsp; - Law of Demand: ↑Price → ↓Quantity demanded  

&nbsp; - Law of Supply: ↑Price → ↑Quantity supplied

\- \*\*Key Formulas\*\*

&nbsp; - GDP (Expenditure Approach):  

&nbsp;   `GDP = C + I + G + (X - M)`  

&nbsp;   Where C = consumption, I = investment, G = government spending, X = exports, M = imports

&nbsp; - Inflation Rate:  

&nbsp;   `Inflation (%) = (CPI this year - CPI last year) / CPI last year × 100`

\- \*\*Market Structures\*\*

&nbsp; - Perfect Competition, Monopoly, Oligopoly, Monopolistic Competition



---



\### \*\*Mathematics\*\*

\- \*\*Algebra\*\*

&nbsp; - Quadratic Formula: `x = (-b ± √(b² - 4ac)) / 2a`

\- \*\*Statistics\*\*

&nbsp; - Mean: `μ = Σx / n`  

&nbsp; - Variance: `σ² = Σ(x - μ)² / n`  

&nbsp; - Standard Deviation: `σ = √σ²`

\- \*\*Probability\*\*

&nbsp; - P(A or B) = P(A) + P(B) - P(A and B)  

&nbsp; - P(A and B) = P(A) × P(B) (if independent)



---



\### \*\*Data Science (Python Basics)\*\*

```python

\# Import libraries

import pandas as pd

import matplotlib.pyplot as plt



\# Load CSV

data = pd.read\_csv('data.csv')



\# View first 5 rows

print(data.head())



\# Plotting example

data\['column\_name'].hist()

plt.show()





=======
# study-resources
Initial commit
# Study Resource Website

Welcome to the **Study Resource Website**, a project designed to provide summarized study materials and tips for various subjects.  
This website is perfect for students looking for a quick reference and links to useful resources for self-learning.

---

## Subjects Covered

### **Economics**
- Key concepts: supply & demand, market structures, fiscal & monetary policies  
- Suggested topics to focus on: GDP, inflation, unemployment, global trade

**Useful Economics Links:**
- [World Bank - Global Economic Data](https://www.worldbank.org/en/research)  
- [International Monetary Fund (IMF) Research](https://www.imf.org/en/Publications)  
- [Investopedia Economics Section](https://www.investopedia.com/economics-4689743)  
- [The Economist – Economic & Financial News](https://www.economist.com/)  
- [Khan Academy – Microeconomics & Macroeconomics](https://www.khanacademy.org/economics-finance-domain)

---

### **Data Science**
- Learn Python programming basics, data analysis with Pandas, and data visualization with Matplotlib & Seaborn  
- Topics include: statistics, machine learning concepts, and exploratory data analysis

**Useful Data Science Links:**
- [Kaggle Learn](https://www.kaggle.com/learn)  
- [DataCamp](https://www.datacamp.com/)  
- [Coursera – Data Science Courses](https://www.coursera.org/browse/data-science)  
- [edX – Data Science Programs](https://www.edx.org/learn/data-science)  
- [Towards Data Science (Articles & Tutorials)](https://towardsdatascience.com/)

---

### **Mathematics**
- Important topics: algebra, calculus, probability, statistics, and formulas for quick reference  
- Focus on practice problems and visualizing concepts

**Useful Mathematics Links:**
- [Paul’s Online Math Notes](https://tutorial.math.lamar.edu/)  
- [Khan Academy – Math Courses](https://www.khanacademy.org/math)  
- [Wolfram Alpha – Solve & Explore Math Problems](https://www.wolframalpha.com/)  
- [Art of Problem Solving](https://artofproblemsolving.com/)  
- [Math is Fun](https://www.mathsisfun.com/)

---

### **Languages**
- Tips for learning Italian, Spanish, German, Polish, Finnish, and more  
- Focus on vocabulary, grammar, and conversational practice

**Useful Language Learning Links:**
- [Duolingo](https://www.duolingo.com/)  
- [BBC Languages](https://www.bbc.co.uk/languages)  
- [Memrise](https://www.memrise.com/)  
- [italki – Find Tutors](https://www.italki.com/)  
- [Clozemaster – Vocabulary Practice](https://www.clozemaster.com/)

---

## Study Tips

- Break study sessions into 25–30 minute intervals with short breaks  
- Focus on **understanding concepts**, not just memorizing  
- Create summary notes and highlight key points  
- Practice problems regularly for subjects like Math & Data Science  
- Review topics multiple times for long-term retention  
- Use online resources to supplement textbooks and lectures  

---

## Further Reading & References

**Economics & Global Studies**
- [OECD – Data & Reports](https://www.oecd.org/)  
- [Federal Reserve Education](https://www.federalreserveeducation.org/)  
- [Economic Times – Global Economy](https://economictimes.indiatimes.com/)

**Data Science & Technology**
- [Python Official Documentation](https://docs.python.org/3/)  
- [Stack Overflow – Q&A](https://stackoverflow.com/)  
- [GitHub – Open Source Data Science Projects](https://github.com/topics/data-science)

**Mathematics & Statistics**
- [Khan Academy Statistics & Probability](https://www.khanacademy.org/math/statistics-probability)  
- [StatTrek – Online Statistics Resources](https://stattrek.com/)  
- [MathWorld – Wolfram](https://mathworld.wolfram.com/)

**Language Learning & Linguistics**
- [Omniglot – Languages & Writing Systems](https://www.omniglot.com/)  
- [LingQ – Language Learning Platform](https://www.lingq.com/)  
- [Language Transfer – Free Audio Lessons](https://www.languagetransfer.org/)

---

## How to View

You can view the live website here:  
[Live Website](https://xthaaman0-beep.github.io/study-rsources/)

---

## Technologies Used

- **HTML** – Structure of the website  
- **CSS** – Styling and layout  
- Optional: Add images, links, and future interactive features  

---

Feel free to explore the code, use the resources, and contribute ideas or improvements.  
This project is meant to grow over time with more content and resources.
---

## Quick Study Cheat Sheet

### **Economics**
- **Supply & Demand**
  - Law of Demand: ↑Price → ↓Quantity demanded  
  - Law of Supply: ↑Price → ↑Quantity supplied
- **Key Formulas**
  - GDP (Expenditure Approach):  
    `GDP = C + I + G + (X - M)`  
    Where C = consumption, I = investment, G = government spending, X = exports, M = imports
  - Inflation Rate:  
    `Inflation (%) = (CPI this year - CPI last year) / CPI last year × 100`
- **Market Structures**
  - Perfect Competition, Monopoly, Oligopoly, Monopolistic Competition

---

### **Mathematics**
- **Algebra**
  - Quadratic Formula: `x = (-b ± √(b² - 4ac)) / 2a`
- **Statistics**
  - Mean: `μ = Σx / n`  
  - Variance: `σ² = Σ(x - μ)² / n`  
  - Standard Deviation: `σ = √σ²`
- **Probability**
  - P(A or B) = P(A) + P(B) - P(A and B)  
  - P(A and B) = P(A) × P(B) (if independent)

---
### **Law**
Explore the legal systems of different countries, key legal principles, and international law.

#### **United States**
- Constitutional Law, Criminal Law, Civil Law, Federal and State systems
- Key resources:
  - [Legal Information Institute (LII)](https://www.law.cornell.edu/) – US law, codes, and case summaries
  - [Oyez](https://www.oyez.org/) – Supreme Court cases and audio recordings
  - [Harvard Law Review](https://harvardlawreview.org/) – Legal research articles

#### **United Kingdom**
- Common law system, constitutional principles, criminal law, civil law
- Key resources:
  - [UK Legislation](https://www.legislation.gov.uk/) – Official acts and statutes
  - [The Supreme Court UK](https://www.supremecourt.uk/) – Landmark judgments
  - [Oxford Journal of Legal Studies](https://academic.oup.com/ojls) – Research articles

#### **India**
- Constitutional law, criminal code, civil procedure, personal law
- Key resources:
  - [India Code](https://www.indiacode.nic.in/) – Full acts and statutes
  - [Supreme Court of India](https://main.sci.gov.in/) – Important judgments
  - [Indian Journal of Legal Research](https://ijlr.in/) – Academic research

#### **Nepal**
- Constitutional law, civil code, criminal code, human rights, local governance
- Key resources:
  - [Nepal Law Commission](https://www.lawcommission.gov.np/) – Full acts, laws, and regulations
  - [Supreme Court of Nepal](http://supremecourt.gov.np/) – Key judgments
  - [Nepal Legal Forum](https://www.nplf.org.np/) – Articles and legal resources
  - [ResearchGate – Nepal Law](https://www.researchgate.net/) – Legal research papers on Nepalese law
- Tips: Focus on **Constitution of Nepal 2015**, Civil and Criminal Codes, and landmark cases.

#### **European Union**
- EU treaties, regulations, directives, EU Court of Justice decisions
- Key resources:
  - [EUR-Lex](https://eur-lex.europa.eu/) – Access to EU law and treaties
  - [European Court of Justice](https://curia.europa.eu/) – Cases and judgments
  - [European Journal of Legal Studies](https://ejls.eui.eu/) – Academic articles

#### **Canada**
- Constitutional law, criminal code, civil law (Quebec), human rights
- Key resources:
  - [CanLII](https://www.canlii.org/) – Canadian laws and court decisions
  - [Supreme Court of Canada](https://www.scc-csc.ca/) – Landmark rulings
  - [Canadian Journal of Law & Society](https://www.cambridge.org/core/journals/canadian-journal-of-law-and-society) – Research articles
 
  - # 📚 Study Resource Cheat Sheet

Welcome! This table contains **all subjects, subtopics, and key resources** for quick reference.

---

| 🏷️ Subject | 🔹 Subtopics / Focus | 🔗 Key Resources |
|------------|--------------------|----------------|
| **💰 Economics** | Supply & Demand, Market Structures, GDP, Inflation, Unemployment, Fiscal & Monetary Policy, Economic Data Analysis, Forecasting, Financial Analytics, Big Data in Economics | [World Bank Data](https://data.worldbank.org/), [Kaggle Economics Datasets](https://www.kaggle.com/datasets?search=economics), [Investopedia](https://www.investopedia.com/economics-4689743), [OECD Data](https://data.oecd.org/) |
| **📊 Data Science** | Python, Pandas, Data Visualization, Machine Learning, EDA, Statistical Analysis, Visualization, ML Models | [Kaggle Learn](https://www.kaggle.com/learn), [Coursera](https://www.coursera.org/browse/data-science), [Analytics Vidhya](https://www.analyticsvidhya.com/) |
| **➗ Mathematics** | Algebra, Calculus, Statistics, Probability, Linear Algebra, Discrete Math | [Khan Academy](https://www.khanacademy.org/math), [Paul’s Online Math Notes](https://tutorial.math.lamar.edu/) |
| **🗣️ Languages** | Italian, Spanish, German, Polish, Finnish, Vocabulary, Grammar, Conversational Practice | [Duolingo](https://www.duolingo.com/), [BBC Languages](https://www.bbc.co.uk/languages) |
| **⚛️ Physics** | Mechanics, Thermodynamics, Electricity & Magnetism, Modern Physics | [Khan Academy](https://www.khanacademy.org/science/physics), [HyperPhysics](http://hyperphysics.phy-astr.gsu.edu/) |
| **💻 Computer Science** | Programming, Algorithms, Data Structures, Networking, Python, Web Dev | [freeCodeCamp](https://www.freecodecamp.org/), [GeeksforGeeks](https://www.geeksforgeeks.org/) |
| **🌍 History & Geography** | World History, Civilizations, Maps, Political Systems | [History.com](https://www.history.com/), [National Geographic Education](https://www.nationalgeographic.org/education/) |
| **🎨 Art & Literature** | Literature Summaries, Famous Authors, Art History, Visual Arts | [Project Gutenberg](https://www.gutenberg.org/), [Google Arts & Culture](https://artsandculture.google.com/) |
| **⚖️ Law – Nepal** | Constitutional Law, Civil & Criminal Codes, Human Rights | [Nepal Law Commission](https://www.lawcommission.gov.np/), [Supreme Court Nepal](http://supremecourt.gov.np/), [Nepal Legal Forum](https://www.nplf.org.np/), [ResearchGate – Nepal Law](https://www.researchgate.net/) |
| **⚖️ Law – USA** | Constitutional, Criminal, Civil Law | [Legal Information Institute](https://www.law.cornell.edu/), [Oyez](https://www.oyez.org/) |
| **⚖️ Law – UK** | Common Law, Criminal Law, Constitutional | [UK Legislation](https://www.legislation.gov.uk/), [Supreme Court UK](https://www.supremecourt.uk/) |
| **⚖️ Law – India** | Constitutional, Criminal, Civil | [India Code](https://www.indiacode.nic.in/), [Supreme Court India](https://main.sci.gov.in/) |
| **⚖️ Law – EU** | Treaties, Regulations, Directives | [EUR-Lex](https://eur-lex.europa.eu/), [European Court of Justice](https://curia.europa.eu/) |
| **⚖️ Law – Canada** | Constitutional, Criminal, Civil | [CanLII](https://www.canlii.org/), [Supreme Court Canada](https://www.scc-csc.ca/) |
| **⚖️ Law – Australia** | Common Law, Constitutional, Criminal & Civil | [AustLII](http://www.austlii.edu.au/), [High Court of Australia](http://www.hcourt.gov.au/) |
| **⚖️ Law – International** | Treaties, Conventions, Human Rights | [UN Treaty Collection](https://treaties.un.org/), [International Court of Justice](https://www.icj-cij.org/), [HeinOnline](https://home.heinonline.org/) |

---

### 💡 Study Tips:
- Focus on **key principles, landmark cases, and datasets** first.  
- Use **Data Science tools** to analyze economic and statistical data.  
- Keep **summary sheets, charts, and cheat notes** for faster revision.  
- Explore provided links for **research articles, official laws, and tutorials**.  
- Continuously add new subjects, topics, or resources as you study.
## Study Tips & Strategies

### **1. Combine Subjects for Better Understanding**
- Use **Data Science tools** to analyze **economic data** or financial trends.  
- Apply **mathematical formulas** to solve real-world problems in Economics and Physics.  
- Explore **historical trends** and **geopolitical factors** when studying Economics or International Law.

### **2. Create Summary Notes & Cheat Sheets**
- Write down **key concepts, formulas, and laws** in one place.  
- Highlight **important terms** using colors or symbols.  
- Use **tables and charts** for quick reference.

### **3. Practice & Review Regularly**
- Solve **problems and exercises** for Mathematics, Data Science, and Economics.  
- Revisit **important cases and legal principles** for Law subjects.  
- Test your understanding by **explaining concepts in your own words**.

### **4. Explore Online Resources**
- Watch **tutorial videos** for complex topics (Khan Academy, Coursera).  
- Read **research papers and articles** for advanced understanding (ResearchGate, HeinOnline).  
- Use **interactive websites** like Duolingo for language learning or Kaggle for data science projects.

### **5. Use Visualizations**
- Create **graphs, charts, and dashboards** to understand data trends.  
- Map out **historical events, political boundaries, or legal systems** visually.  
- Visualization makes memorization easier and learning faster.

### **6. Organize Study Sessions**
- Use **Pomodoro technique**: 25–30 minutes of focused study, 5-minute breaks.  
- Focus on **one subject at a time**, but mix topics weekly for better retention.  
- Track your **progress and revisit difficult topics** regularly.

### **7. Extra Resources**
- **Economic Research & Data:** [IMF Data](https://www.imf.org/en/Data), [World Bank Data](https://data.worldbank.org/)  
- **Legal Research:** [HeinOnline](https://home.heinonline.org/), [Globalex](https://www.nyulawglobal.org/globalex/)  
- **Programming & Data Science:** [Kaggle](https://www.kaggle.com/), [Analytics Vidhya](https://www.analyticsvidhya.com/)  
- **History & Geography:** [National Geographic Education](https://www.nationalgeographic.org/education/), [History.com](https://www.history.com/)  

---

💡 **Pro Tip:**  
Combine **notes, visualization, and practice** to create a personalized study routine. This approach will save time, enhance understanding, and improve long-term retention.
>>>>>>> 5dd7303a33d41459f1170f20407cb5dbdb302de3


